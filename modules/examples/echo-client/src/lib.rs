...
impl EchoClient {
    ...
    fn on_block(block: types::Block) -> Result<(), Fault> {
        ...
        match client::quote(ECHO_VENUE, &body) {
            Ok(quotation) => tracing::info!(
                "quoted {} bytes at {ECHO_VENUE}: gives {} amount bytes",
                body.len(),
                quotation.gives.amount.len(),
            ),
            Err(_) => tracing::warn!("quote at {ECHO_VENUE} was refused"),
        }
        match client::submit(ECHO_VENUE, &body) {
            Ok(SubmitOutcome::Accepted(receipt)) => tracing::info!(
                "submitted {} bytes to {ECHO_VENUE}, receipt {} bytes",
                body.len(),
                receipt.len(),
            ),
            Ok(SubmitOutcome::RequiresSigning(_)) => {
                tracing::warn!("{ECHO_VENUE} unexpectedly asked for a signature")
            }
            Err(_) => tracing::warn!("submit to {ECHO_VENUE} was refused"),
        }
        Ok(())
    }
