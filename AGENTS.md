...
Do not use a banner or a section-divider comment.
A section is a module boundary: split the file or add nothing, because `.claude/hooks/content-lint.sh` blocks a new banner comment in a `.rs` or `.md` file.
