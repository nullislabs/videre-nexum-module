...
//! `wasmtime::component::bindgen!`. The shared `nexum:host` interfaces are
//! reused from the runtime bindings via `with`, and the client bindgen
...
/// speak one `SubmitOutcome`/`VenueError`. Async: the `Host` impl awaits the
/// per-adapter mutex and the adapter's guest calls.
mod client_host {
    wasmtime::component::bindgen!({
        inline: "
...
/// The host-bound client interface: the `Host` trait and `add_to_linker`.
pub use client_host::videre::venue::client;
/// The shared intent ontology at its plain spellings.
...
/// Operator-log rendering of a wire `venue-error`, preserving the
/// rate-limit `retry-after-ms` hint.
...
/// Identifier-hygiene smoke for `videre:value-flow`: naming every generated
/// type by its plain Rust spelling fails the build on a keyword collision
...
/// Identifier-hygiene and signature smoke for `videre:types`/`videre:venue`:
/// naming every generated type and pinning the five `client` signatures
/// fails the build on a keyword collision or signature change.
#[cfg(test)]
mod client_smoke {
    wasmtime::component::bindgen!({
        inline: "
