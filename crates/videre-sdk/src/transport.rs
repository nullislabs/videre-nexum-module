...
/// Per-request timeout middleware over any [`Fetch`]: every request
/// through it, including plain [`Fetch::fetch`], carries phase timeouts
/// clamped to the configured bound, so a hung endpoint errors within it
/// rather than stalling the adapter's export call.
#[derive(Clone, Copy, Debug)]
pub struct TimedFetch<F> {
    inner: F,
    bound: Duration,
}
...
impl<F: Fetch> Fetch for TimedFetch<F> {
    fn fetch_with(
        &self,
