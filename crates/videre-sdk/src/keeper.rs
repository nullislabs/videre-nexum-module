...
impl<S, P: VenueTransport> Keeper<S, P> {
    ...
    /// best-effort - so an accepted body never reaches the venue
    /// twice; a `requires-signing` answer journals nothing and is
    /// surfaced afresh each sweep. Store faults abort the sweep, bar
    /// the post-acceptance marker clear; venue refusals never do -
    /// they fold into per-watch retry actions.
