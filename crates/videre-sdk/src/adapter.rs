...
//!
//! The trait mirrors the world's export face one to one. Functions are
//! associated (no `self`): the component model calls exports statically,
//! so adapter state lives in the adapter's own statics.
...
/// `venue-adapter` world. Implement on a unit struct and apply
/// [`#[videre_sdk::venue]`](crate::venue) to the impl; bodies and
/// receipts arrive as opaque bytes, typed through
/// [`IntentBody`](crate::IntentBody).
pub trait VenueAdapter {
    ...
    /// equals the manifest `[venue] body_versions` set. Defaults to none.
    fn body_versions() -> Vec<u32> {
        Vec::new()
    }

    /// Project an opaque body onto the header guard policy runs on. A pure
    /// derivation: no transport, no side effects.
    fn derive_header(body: Vec<u8>) -> Result<IntentHeader, VenueError>;

    /// Price an opaque intent body: indicative, not an offer the venue is
    /// bound to fill.
    fn quote(body: Vec<u8>) -> Result<Quotation, VenueError>;

    /// Submit an opaque intent body. Success is the venue's receipt or
    /// `requires-signing`, a transaction the host must sign and send
    ...
    /// export shim rejects an empty receipt as `invalid-receipt`.
    fn status(receipt: Vec<u8>) -> Result<IntentStatus, VenueError>;

    /// Ask the venue to withdraw an intent. Success means the venue
    /// accepted the cancellation, not that an in-flight settlement can no
    /// longer win the race. The export shim rejects an empty receipt.
    fn cancel(receipt: Vec<u8>) -> Result<(), VenueError>;
}

/// Internal codegen `#[videre_sdk::venue]` expands to: a hidden shim
/// wiring a [`VenueAdapter`] impl to the world's `Guest` faces, then its
/// `export!`. Meaningful only inside the attribute's output, where the
/// world bindgen resolves `Guest`, `exports`, and `export!`.
#[doc(hidden)]
#[macro_export]
